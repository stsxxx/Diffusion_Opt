...
class NodeWorker:
    ...
    def fetch_and_process_requests(self):
        ...
        log_file = f"SLO_LOG/Ours_slo_{args.rate}_3000.log"
        while self.running:
            try:
                ...
                if request['k'] is None:
                    ...
                    generated_image_path = f"/work1/talati/stilex/diffusion_proj/multinode/SLO/Ours/{clean_prompt}.png"
                    try:
                    ...
                    try:
                        ...
                        if self.model_type == "large":
                            ...
                            wait_time = request['strength'] * 16.5
                            time.sleep(wait_time)
                            end_time = rpc.rpc_sync("scheduler", get_current_time)
                            finish_time = end_time - request['start_time']
                            with open(log_file,"a") as f:
...
class Scheduler:
    ...
    def process_request(self, selected_requests, start_time):
        ...
        small_model_throughput = 1 / (16.5/60)  # Requests/min
...
if __name__ == "__main__":
    ...
    if int(os.environ["SLURM_PROCID"]) == 0:
        ...
        metadata_df = pd.read_parquet('/work1/talati/stilex/diffusion_proj/metadata.parquet')
        device = "cuda:0"
        ...
        final_image_embeddings = torch.load("/work1/talati/stilex/diffusion_proj/multinode/throughput/final_image_embeddings.pt", map_location=device)
