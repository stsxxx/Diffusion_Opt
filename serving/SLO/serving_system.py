...
def request_scheduler(
    ...
    cached_image_paths, shared_new_images, lock,  model, processor,worker_status,  log_file,  cache_size=100000
):
...
def worker(gpu_id, hit_queue, nonhit_queue, control_queue, shared_new_images,latency_queue ,worker_status):
    ...
    idle_counter = 0
    max_idle_iterations = 90 # Set a threshold for termination
    ...
    while True:
        try:
            ...
            callback_on_step_end_tensor_inputs=["latents"])
                ...
                generated_image_path = f"/data6/stilex/SLO/ours/{clean_prompt}.png"
...
if __name__ == "__main__":
    ...
    lock = mp.Lock()
    log_file=f"request_throughput_{num_requests}_rr{args.rate}_Ours.csv"

    # Start request scheduler process
    scheduler = mp.Process(target=request_scheduler, args=(
        hit_queue, nonhit_queue, selected_requests, start_time, request_rate_queue, final_image_embeddings, cached_image_paths, shared_new_images, lock, model, processor, worker_status, log_file
    ))
    scheduler.start()
