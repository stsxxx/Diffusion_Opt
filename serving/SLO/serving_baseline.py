...
# Request scheduler
def request_scheduler(req_queue,  selected_requests, start_time, log_file, worker_status):
...
# Worker Process
def worker(gpu_id, req_queue, latency_queue, worker_status):
    ...
    while True:
        try:
            ...
            generated_image_path = f"/data6/stilex/SLO/baseline/{clean_prompt}.png"
...
if __name__ == "__main__":
    ...
    log_file=f"request_throughput_{num_requests}_rr{args.rate}_Baseline.csv"
    
    start_time = time.time()

    scheduler = mp.Process(target=request_scheduler, args=(req_queue,  selected_requests, start_time, log_file, worker_status))
    ...
    for gpu_id in range(num_gpus):
        ...
        p = mp.Process(target=worker, args=(gpu_id, req_queue, latency_queue, worker_status))
        p.start()
        workers.append(p)
