...
import pandas as pd
from diffusers import StableDiffusion3Pipeline, StableDiffusionXLImg2ImgPipeline, SanaPipeline
...
def request_scheduler(
    ...
    cached_image_paths, shared_new_images, lock,  model, processor,worker_status, scheduler_status, cache_size=100000,  log_file="request_throughput_5000_ours_SANA.csv"
):
    ...
    for _, row in sorted_df.iterrows():
        ...
        if k is not None:
            ...
            print("retrieved image:", cached_image_paths[best_index])
        ...
        #     k_rates = {k: count / request_count for k, count in k_distribution.items()}  # Normalize k counts
...
def global_monitor(request_rate_queue, control_queues, num_gpus, total_num_steps=50):
    ...
    pid_controller = PIDController(Kp, Ki, Kd, setpoint=0, min_output=1, max_output=num_gpus)
    ...
    while True:
        try:
            ...
            current_time = time.time()
            dt = current_time - prev_time  # Time since last adjustment
    ...
    while True:
        try:
            ...
            callback_on_step_end_tensor_inputs=["latents"])
                ...
                generated_image_path = f"/data6/stilex/throughput/SANA/{clean_prompt}.png"
            ...
            idle_counter += 1
            if  scheduler_status["status"] == "dropped" and model_type == "sana" and hit_queue.empty():
                model_type = "sd3.5"
                model = load_model(model_type, device)
...
if __name__ == "__main__":
    ...
    scheduler = mp.Process(target=request_scheduler, args=(
        hit_queue, nonhit_queue, selected_requests, start_time, request_rate_queue, final_image_embeddings, cached_image_paths, shared_new_images, lock, model, processor, worker_status, scheduler_status
    ))
    scheduler.start()
